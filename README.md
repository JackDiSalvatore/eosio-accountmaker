...
To run a full boot of the eosio system contracts (eosio.contractsv1.4.0),
 ...
 inside the 'eosio_accountmaker_container' docker run
```bash
...
```sh
docker exec -it eosio_accountmaker_container bash
```
...
```sh
docker run --rm --name eosio_accountmaker_container \
